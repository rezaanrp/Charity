...
namespace CharityTestCore.Service.EPT
{
    ...
    {
             private readonly IEPTRepository repository;

        public EPTService(IEPTRepository eptrepository)
        {

            this.repository = eptrepository;

        }
        public int AddEptQuestion( string UserId, bool? HaveSkill_, bool? BeforeYouHaveJob_, bool? DoYouWantHaveJob_,
        ...
        public EPTListPersonModel EptPersonList(Guid? id)
        {
        ...
        public EptQuestionList? EptPersonById(Guid? id)
        {
            return repository.EptQuestion.FirstOrDefault(x => x.Id == id);

        }
        public bool EptPersonDeleteById(Guid? ept)
        {
            try
            {
        ...
        public List<EPTQuizTextModel> EptQuizTextList()
        {
